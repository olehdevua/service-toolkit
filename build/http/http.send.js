...
import { GeneralError, HttpError, NotFoundError } from "../errors.js";
...
export class FileSender {
    ...
    logger;
    opts;
    constructor(req, logger, opts) {
        this.req = req;
        this.logger = logger;
        this.opts = opts;
    }
    static init(req, logger, options) {
    ...
    async handleCaching(res, path, stat) {
    ...
    async sendError(res, err) {
        ...
        if (err && Object(err.params?.headers) === err.params?.headers) {
            setHeaders(res, err.params?.headers);
        }
...
function normalizePath(path, root) {
    if (typeof root === "string") {
        ...
        const finPath = resolve(path);
        return [finPath, parts];
    }
}
function assertDotFilesAllowed(parts, dotFileAction, path) {
...
class SendStreamOptsValidator extends Validator {
    checkDotfiles(dotfiles) {
        const strDotfiles = this.checkString(dotfiles, "dotfiles");
        if (this.getError("dotfiles"))
            return;
        if (strDotfiles === "ignore" || strDotfiles === "allow" || strDotfiles === "deny") {
            return strDotfiles;
        }
        this.setError("dotfiles", new TypeError("dotfiles option must be \"allow\", \"deny\", or \"ignore\""));
    }
    castMaxAge(maxage = MAX_MAXAGE) {
        let finalMaxAge;
        finalMaxAge = this.castRange(maxage, "maxage", { min: 0, max: MAX_MAXAGE });
        if (!this.getError("maxage"))
            return finalMaxAge;
        this.cleanError("maxage");
        finalMaxAge = this.checkString(maxage, "maxage");
        if (this.getError("maxage"))
            return undefined;
        finalMaxAge = ms(finalMaxAge);
    ...
    castPath(path) {
        ...
        if (this.getError("path"))
            return;
        let decodedPath;
        try {
            decodedPath = decodeURIComponent(strPath);
        }
        catch (err) {
            this.setError("path", { message: err.message });
            return;
        }
    ...
    validate(options) {
        const opts = {
            ...
            lastModified: true,
            ...options,
            root: this.checkString(options.root, "root"),
            dotfiles: this.checkDotfiles(options.dotfiles || "ignore"),
            maxage: this.castMaxAge(options.maxage),
            path: this.castPath(options.path),
        };
...
function mapStatError(error, path) {
    switch (error.code) {
        case "ENAMETOOLONG":
        case "ENOENT":
        case "ENOTDIR":
