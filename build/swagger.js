...
`;
export class OpenAPIAssetsController {
    logger;
    swaggerInitJS;
    sendOpts;
    constructor(logger, swaggerInitJS, sendOpts) {
        this.logger = logger;
        this.swaggerInitJS = swaggerInitJS;
        this.sendOpts = sendOpts;
    }
    static init(logger, sendOpts, swaggerInitJS) {
        const root = swaggerUi.getAbsoluteFSPath();
        return new OpenAPIAssetsController(logger, swaggerInitJS, { ...sendOpts, root });
    }
    async handle(req, resp, { context }) {
        const logger = this.logger.clone({ context });
        const path = h.getPath(req);
        logger.debug("Swagger asset path", { path });
        if (path === "/swagger-ui-init.js") {
            return h.sendText(resp, this.swaggerInitJS, {
                status: 200,
                type: "application/javascript"
            });
        }
        const ss = h.FileSender.init(req, logger, { ...this.sendOpts, path });
        await ss.send(resp);
    }
}
